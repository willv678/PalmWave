    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                left_click_distance = ((index_x - thumb_x) ** 2 + (index_y - thumb_y) ** 2) ** 0.5
                right_click_distance = ((middle_x - thumb_x) ** 2 + (middle_y - thumb_y) ** 2) ** 0.5

                # Print a message when the index and thumb are close
                if left_click_distance < CLICK_DISTANCE_THRESHOLD:
                    print("Click detected!")
                    perform_click()
                
                if right_click_distance < CLICK_DISTANCE_THRESHOLD:
                    print("Right click detected!")
                    perform_right__click()
                ...
                cv2.circle(frame, (middle_x, middle_y), 5, (100, 200, 0), -1)
                

        # Display the frame with overlays
        cv2.imshow("PalmWave Camera View", frame)
